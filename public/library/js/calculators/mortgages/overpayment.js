	...
	c.extend(	
	{
		...
		funcs:{
			...
			setupInputs: function() {
				...
				_CORE.utils.blurOnKeyPress(_CORE.refs["OVERPAYMENT-CALC_mortgage_balance"]);
				...
				_CORE.utils.blurOnKeyPress(_CORE.refs["OVERPAYMENT-CALC_mortgage_term_years"]);
				...
				_CORE.utils.blurOnKeyPress(_CORE.refs["OVERPAYMENT-CALC_mortgage_term_months"]);
				...
				_CORE.utils.blurOnKeyPress(_CORE.refs["OVERPAYMENT-CALC_mortgage_rate"]);
				...
				_CORE.utils.blurOnKeyPress(_CORE.refs["OVERPAYMENT-CALC_regular_overpayment"]);
				...
				_CORE.utils.blurOnKeyPress(_CORE.refs["OVERPAYMENT-CALC_lump_sum_overpayment"]);
...
*/
			...
			currentPaymentCalc: function(){
				...
				if(isNaN(result) || result == 0){
					...
					let paymentResult = Number(result).toLocaleString("en-GB", {style:"currency", currency:"GBP"});

					_CORE.overpayment.cache.currentPaymentValue = result;

					console.log(paymentResult);

					_CORE.refs["OVERPAYMENT-CALC_mortgage-current-monthly-payment"].textContent = paymentResult;

					_CORE.refs["OVERPAYMENT-CALC_mortgage-new-monthly-payment"].textContent = paymentResult;

					_CORE.overpayment.funcs.newPaymentCalc(paymentResult);
			...
			newPaymentCalc: function(currentPayment){
				...
				if(currentPayment == 0){
					...
					currentPayment = currentPayment.replaceAll("£", "");
			...
			calculateTableValues: function(){
				...
				_CORE.overpayment.cache.monthlyBalanceResults = [];
				_CORE.overpayment.cache.monthlyBalanceResults.push(['Month', 'Without Overpayment', 'With Overpayment']);
				...
				_CORE.overpayment.cache.monthlyBalanceResults.push([0,Math.trunc(Number(mortgageBalance)),  Math.trunc(Number(mortgageBalance))]);
					...
					for(let y = 0; y < 12; y++){
						...
						if(i == 0 && y == 0){
							...
							if(y == 11){
								...
								if(withOpBalance <= 100 ){
									withOpBalance = 0;
								} 

								if(withoutOpBalance <= 100 ){
									withoutOpBalance = 0;
								} 
						...
						_CORE.overpayment.cache.monthlyBalanceResults.push([currentMonth ,Math.trunc(Number(withoutOpBalance)),  Math.trunc(Number(withOpBalance))]);
				...
				if(_CORE.overpayment.cache.table_view == 'yearly'){
					...
					_CORE.utils.forEach(results, function(index, arr){
						if(index == 0 || index > (results.length)){
							...
							_CORE.overpayment.funcs.buildTableRow(index, 0, 0);					
						} else {
							let month = arr[0];
							let withoutOp = Math.trunc(Number(arr[1]));
							let withOp = Math.trunc(Number(arr[2]));
	
							_CORE.overpayment.funcs.buildTableRow(month, withOp, withoutOp);
						}
					});
				}
			},
