<div class="uk-animation-slide-left-small">
    <div class="uk-card uk-card-default fd--calc--list--cards">
        <div class="fd--card--header uk-padding-small">
            <h4>Credit Card Calculator</h4>
        </div>
        <div class="fd--card--body uk-padding">
            <p>
                A credit card calculator is a tool that helps you estimate the cost of borrowing money on a credit card. It calculates the interest charges and the total cost of credit based on the balance on the card, the interest rate, and the minimum payment required each month.
            </p>
        </div>
        <div class="fd--card--footer uk-animation-toggle">
            <button class="uk-button" type="button" onclick="_CORE.funcs.switchLoanCalc()">
                GO
                <span uk-icon="icon: arrow-right; ratio: 1.8" class="uk-animation-slide-left"></span>
            </button>
        </div>
    </div>
</div>
<!--
<div class="uk-animation-slide-left-small">
    <div class="uk-card uk-card-default fd--calc--list--cards">
        <div class="fd--card--header uk-padding-small">
            <h4>Credit Card Overpayment Calculator</h4>
        </div>
        <div class="fd--card--body uk-padding">
            <p>
                More to come
            </p>
        </div>
        <div class="fd--card--footer uk-animation-toggle">
            <button class="uk-button" type="button" onclick="_CORE.funcs.switchLoanCalc()">
                GO
                <span uk-icon="icon: arrow-right; ratio: 1.8" class="uk-animation-slide-left"></span>
            </button>
        </div>
    </div>
</div>
-->
<div class="uk-animation-slide-left-small">
    <div class="uk-card uk-card-default fd--calc--list--cards">
        <div class="fd--card--header uk-padding-small">
            <h4>Loan Calculator</h4>
        </div>
        <div class="fd--card--body uk-padding">
            <p>
                A loan calculator is a tool that helps you estimate the cost of borrowing money. It calculates the monthly payments and the total amount of interest you will pay on a loan, based on the loan amount, interest rate, and loan duration.
            </p>
        </div>
        <div class="fd--card--footer uk-animation-toggle">
            <button class="uk-button" type="button" onclick="_CORE.funcs.switchLoanCalc()">
                GO
                <span uk-icon="icon: arrow-right; ratio: 1.8" class="uk-animation-slide-left"></span>
            </button>
        </div>
    </div>
</div>

<div class="uk-animation-slide-left-small">
    <div class="uk-card uk-card-default fd--calc--list--cards">
        <div class="fd--temp--overlay">
            <div class="fd--temp--overlay--text">Coming Soon</div>
        </div>
        <div class="fd--card--header uk-padding-small">
            <h4>Loan Overpayment Calculator</h4>
        </div>
        <div class="fd--card--body uk-padding">
            <p>
                The calculator can show you how much interest you can save and how much faster you can pay off the loan by making overpayments. Using a loan overpayment calculator can help you make informed decisions about whether or not to make extra payments on your loan, and how much to pay.
            </p>
        </div>
        <div class="fd--card--footer uk-animation-toggle">
            <button class="uk-button" type="button" onclick="">
                GO
                <span uk-icon="icon: arrow-right; ratio: 1.8" class="uk-animation-slide-left"></span>
            </button>
        </div>
    </div>
</div>

